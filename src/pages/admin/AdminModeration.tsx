...
import { AlertTriangle, Eye, EyeOff, ChevronRight, ArrowLeft, Search } from 'lucide-react';
...
export const AdminModeration = () => {
  ...
  // Filter state for titles list
  const [titleSearchQuery, setTitleSearchQuery] = useState('');

  // Filter state for reviews list
  const [reviewStatusFilter, setReviewStatusFilter] = useState<'all' | 'visible' | 'hidden'>('all');
  ...
  const filteredTitles = useMemo(() => {
    if (!titlesWithReviews) return [];
    
    return titlesWithReviews.filter(title => {
      if (titleSearchQuery && !title.name.toLowerCase().includes(titleSearchQuery.toLowerCase())) {
        return false;
      }
      return true;
    });
  }, [titlesWithReviews, titleSearchQuery]);

  // Filter reviews
  const filteredReviews = useMemo(() => {
    if (!titleReviews) return [];
    
    return titleReviews.filter(review => {
      if (reviewStatusFilter === 'visible' && review.is_deleted) return false;
      if (reviewStatusFilter === 'hidden' && !review.is_deleted) return false;
      return true;
    });
  }, [titleReviews, reviewStatusFilter]);
          ...
          <Card>
            <CardContent className="text-center py-12">
              <p className="text-muted-foreground">
                {titleSearchQuery 
                  ? 'Nenhum título encontrado com a busca aplicada.' 
                  : 'Nenhum título com avaliações encontrado.'}
              </p>
            </CardContent>
          </Card>
        )}
      ...
      {filteredReviews.length === 0 && (
        <Card>
          <CardContent className="text-center py-12">
            <p className="text-muted-foreground">
              {reviewStatusFilter !== 'all'
                ? 'Nenhuma avaliação encontrada com o filtro aplicado.'
                : 'Nenhuma avaliação encontrada para este título.'}
            </p>
          </CardContent>
        </Card>
      )}

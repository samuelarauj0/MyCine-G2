                ...
                <Card className="shadow-card">
                  <CardContent className="p-3 md:p-4">
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-sm md:text-base">{stats.totalUsers.toLocaleString()}</div>
                ...
                <Card className="shadow-card">
                  <CardContent className="p-3 md:p-4">
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-sm md:text-base">{stats.totalReviews.toLocaleString()}</div>
                ...
                <Card className="shadow-card">
                  <CardContent className="p-3 md:p-4">
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-sm md:text-base">{stats.avgRating}</div>
                ...
                <Card className="shadow-card">
                  <CardContent className="p-3 md:p-4">
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-sm md:text-base">{stats.legendUsers}</div>
                        <div className="text-xs md:text-sm text-muted-foreground">Diamantes</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <CardContent>
                  <div className="space-y-2 md:space-y-4">
                    {topUsers.map((user, index) => (
                      <div 
                        key={user.id}
                        className="flex items-center gap-2 md:gap-4 p-3 md:p-4 rounded-lg hover:bg-muted/50 transition-colors"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-semibold text-sm md:text-base truncate">{user.display_name}</h4>
                        ...
                        <div className="flex flex-col items-end gap-1 flex-shrink-0">
                          ...
                          <div className="text-right">
                            <div className="font-semibold text-xs md:text-sm">{user.total_xp.toLocaleString()} XP</div>
                            <div className="text-xs text-muted-foreground hidden md:block">Nível {user.level}</div>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    {currentUserData && !topUsers.some(u => u.id === user?.id) && (
                      <div className="border-t pt-4 mt-6">
                        <div className="flex items-center gap-2 md:gap-4 p-3 md:p-4 rounded-lg bg-primary/5 border border-primary/20">
                          <div className="flex-shrink-0 w-6 md:w-8 flex justify-center">
                            <span className="text-xs md:text-sm font-bold">#{currentUserData.position}</span>
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="font-semibold text-sm md:text-base truncate">{currentUserData.display_name}</h4>
                            <div className="text-xs md:text-sm text-muted-foreground">
                          ...
                          <div className="flex flex-col items-end gap-1 flex-shrink-0">
                            <Badge className={`${getRankColor(currentUserData.rank)} text-xs`}>
                              {currentUserData.rank}
                            </Badge>
                            <div className="text-right">
                              <div className="font-semibold text-xs md:text-sm">{currentUserData.total_xp.toLocaleString()} XP</div>
                              <div className="text-xs text-muted-foreground hidden md:block">Nível {currentUserData.level}</div>
                            </div>
                          </div>
                        </div>
                      </div>
